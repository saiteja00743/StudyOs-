...
function parseMarkdown(content: string): React.ReactNode[] {
  ...
  segments.forEach((segment, si) => {
    if (segment.startsWith('```')) {
      ...
      nodes.push(
        <div key={`code-${si}`} className="my-3 rounded-xl overflow-hidden border border-white/10 bg-[#161514] shadow-inner">
          <div className="flex items-center justify-between px-4 py-2 bg-[#211f1d] border-b border-white/10">
            <span className="text-xs font-semibold text-brand-400 uppercase tracking-wider">{lang || 'code'}</span>
            <button
          ...
          <pre className="p-4 overflow-x-auto text-sm text-stone-200 leading-relaxed font-mono whitespace-pre">
            <code>{code}</code>
          </pre>
        </div>
      );
    ...
    while (i < lines.length) {
      ...
      if (trimmed.startsWith('### ')) {
        nodes.push(
          <h3 key={`h3-${si}-${i}`} className="text-sm font-bold text-stone-100 mt-2 mb-1">
      ...
      if (trimmed.startsWith('> ')) {
        nodes.push(
          <blockquote key={`bq-${si}-${i}`} className="border-l-2 border-brand-500 pl-3 my-2 text-stone-300 italic bg-brand-500/10 py-1.5 rounded-r-lg text-sm">
            {renderInline(trimmed.slice(2))}
          </blockquote>
        );
        i++;
      ...
      if (/^[-*+]\s/.test(trimmed)) {
        ...
        nodes.push(
          <ul key={`ul-${si}-${i}`} className="space-y-1 my-1.5 pl-4">
            {items.map((item, ii) => (
              <li key={ii} className="text-sm text-stone-200 leading-relaxed flex gap-2">
      ...
      if (/^\d+\.\s/.test(trimmed)) {
        ...
        nodes.push(
          <ol key={`ol-${si}-${i}`} className="space-y-1 my-1.5 pl-4">
            {items.map((item, ii) => (
              <li key={ii} className="text-sm text-stone-200 leading-relaxed flex gap-2">
                <span className="text-brand-400 font-semibold flex-shrink-0 min-w-[1rem]">{ii + 1}.</span>
                <span>{renderInline(item)}</span>
              </li>
            ))}
          </ol>
      ...
      // Regular paragraph
      nodes.push(
        <p key={`p-${si}-${i}`} className="text-sm text-stone-200 leading-relaxed">
...
function renderInline(text: string): React.ReactNode {
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    if (match[2]) {
      ...
      // Italic *text*
      parts.push(<em key={match.index} className="italic text-stone-300">{match[4]}</em>);
    } else if (match[5]) {
      // Inline code `code`
      parts.push(
        <code key={match.index} className="bg-white/10 text-brand-300 px-1.5 py-0.5 rounded text-xs font-mono">
          {match[5]}
        </code>
      );
    ...
    return (
      ...
      >
        <div className="max-w-[80%] lg:max-w-[70%]">
          <div className="bg-brand-500 text-white rounded-2xl rounded-br-sm px-4 py-3 text-sm leading-relaxed shadow-lg">
            {message.content}
          </div>
          <div className="text-right text-xs text-stone-500 mt-1 mr-1">{message.timestamp}</div>
        </div>
        <div className="w-8 h-8 rounded-full bg-surface-800 border border-white/10 flex items-center justify-center flex-shrink-0 mt-1">
        ...
        <div className={cn(
          ...
          {message.isStreaming ? (
            <div className="space-y-2">
              ...
              <span className="inline-block w-2 h-4 bg-brand-400 rounded-sm animate-pulse ml-0.5" />
            </div>
          ) : (
            <MarkdownRenderer content={message.content} />
          )}
        ...
        {!message.isStreaming && (
          <div className="flex items-center gap-4 mt-2 ml-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
            ...
            <button
              onClick={handleSpeak}
              className={cn(
                'flex items-center gap-1 text-xs text-stone-500 hover:text-stone-200 transition-colors',
                speaking && 'text-brand-400'
              )}
            >
